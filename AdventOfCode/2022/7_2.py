...
def main(file):
    ...
    print(fs)
    total_used = fs.size_map[f'None -> {fs.root.name}']
    avail_space = TOTAL_SPACE - total_used
    needed_space = UPD_BUFFER - avail_space
    print(f'Update buffer: {UPD_BUFFER}\nTotal used: {total_used}\nAvil space: {avail_space}\nNeeded space: {needed_space}')
    ans = UPD_BUFFER
    for v in fs.size_map.values():
    	if v < needed_space:
    		continue
    	ans = min(ans, v)
    return ans
